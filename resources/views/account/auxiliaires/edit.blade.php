    ...
    <div
        ...
        <div class="px-10 mt-6">
            ...
            <div class="container m-4">
                <h1 class="mb-4">Modifier l'Auxiliaire</h1>
                @if (session('success'))
            ...
            @endif
                <div class="card">
                    <div class="card-header">
                        Modifier  l'Auxiliaire
                    </div>
                    <div class="card-body">
                        <form action="{{ route('account.auxiliaires.update', $auxiliaire->id) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
            
                            <div class="mb-3">
                                <label for="Nom_Fr" class="form-label">Nom (Français)</label>
                                <input type="text" class="form-control" id="Nom_Fr" name="Nom_Fr" value="{{ $auxiliaire->Nom_Fr }}" required>
                            </div>
            
                            <div class="mb-3">
                                <label for="Prenom_Fr" class="form-label">Prénom (Français)</label>
                                <input type="text" class="form-control" id="Prenom_Fr" name="Prenom_Fr" value="{{ $auxiliaire->Prenom_Fr }}" required>
                            </div>
            
                            <div class="mb-3">
                                <label for="Nom_Ar" class="form-label">Nom (Arabe)</label>
                                <input type="text" class="form-control" id="Nom_Ar" name="Nom_Ar" value="{{ $auxiliaire->Nom_Ar }}" required>
                            </div>
            
                            <div class="mb-3">
                                <label for="Prenom_Ar" class="form-label">Prénom (Arabe)</label>
                                <input type="text" class="form-control" id="Prenom_Ar" name="Prenom_Ar" value="{{ $auxiliaire->Prenom_Ar }}" required>
                            </div>
            
                            <div class="mb-3">
                                <label for="Email" class="form-label">Email</label>
                                <input type="email" class="form-control" id="Email" name="Email" value="{{ $auxiliaire->Email }}" required>
                            </div>
            
                            <div class="mb-3">
                                <label for="Grade" class="form-label">Grade</label>
                                <input type="text" class="form-control" id="Grade" name="Grade" value="{{ $auxiliaire->Grade }}" required>
                            </div>
            
                            <div class="mb-3">
                                <label for="CNIE" class="form-label">CNIE</label>
                                <input type="text" class="form-control" id="CNIE" name="CNIE" value="{{ $auxiliaire->CNIE }}" required>
                            </div>
            
                            <div class="mb-3">
                                <label for="url_photo" class="form-label">Photo Url</label>
                                <input type="text" class="form-control" value="{{ $auxiliaire->url_photo }}" id="url_photo" name="url_photo" >
                            </div>
            
                            <div class="mb-3">
                                <label for="RIB" class="form-label">RIB</label>
                                <input type="text" class="form-control" id="RIB" name="RIB" value="{{ $auxiliaire->RIB }}" required>
                            </div>
            
                            <div class="mb-3">
                                <label for="date_de_naissance" class="form-label">Date de Naissance</label>
                                <input type="date" class="form-control" id="date_de_naissance" name="date_de_naissance" value="{{ $auxiliaire->date_de_naissance }}" required>
                            </div>
            
                            <div class="mb-3">
                                <label for="date_de_recrutement" class="form-label">Date de Recrutement</label>
                                <input type="date" class="form-control" id="date_de_recrutement" name="date_de_recrutement" value="{{ $auxiliaire->date_de_recrutement }}" required>
                            </div>
            
                            <div class="mb-3">
                                <label for="Type" class="form-label">Type</label>
                                <select class="form-control" id="Type" name="Type" required>
                                    <option value="rural" {{ $auxiliaire->Type == 'rural' ? 'selected' : '' }}>Rural</option>
                                    <option value="urbain" {{ $auxiliaire->Type == 'urbain' ? 'selected' : '' }}>Urbain</option>
                                </select>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-control" id="pensionne" name="pensionne" required>
                                    <option value="0" {{ $auxiliaire->pensionne == 0 ? 'selected' : '' }}>Non</option>
                                    <option value="1" {{ $auxiliaire->pensionne == 1 ? 'selected' : '' }}>Oui</option>
                                </select>
                            </div>
                            ...
                            <button type="submit" class="btn btn-primary">Mettre à Jour l'Auxiliaire</button>
                        </form>
                    </div>
                </div>
            </div>
