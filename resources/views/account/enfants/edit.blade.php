        ...
        <div class="px-10 mt-6">
            ...
            <div class="container m-4">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <form action="{{ route('account.enfants.update', $enfant->id) }}" method="POST">
                            @csrf
                            @method('PUT')
            
                            <div class="mb-3">
                                <label for="Nom_Fr" class="form-label">Nom (Français)</label>
                                <input type="text" class="form-control" id="Nom_Fr" name="Nom_Fr" value="{{ $enfant->Nom_Fr }}" required>
                            </div>
            
                            <div class="mb-3">
                                <label for="Prenom_Fr" class="form-label">Prénom (Français)</label>
                                <input type="text" class="form-control" id="Prenom_Fr" name="Prenom_Fr" value="{{ $enfant->Prenom_Fr }}" required>
                            </div>
            
                            <div class="mb-3">
                                <label for="Nom_Ar" class="form-label">Nom (Arabe)</label>
                                <input type="text" class="form-control" id="Nom_Ar" name="Nom_Ar" value="{{ $enfant->Nom_Ar }}" required>
                            </div>
            
                            <div class="mb-3">
                                <label for="Prenom_Ar" class="form-label">Prénom (Arabe)</label>
                                <input type="text" class="form-control" id="Prenom_Ar" name="Prenom_Ar" value="{{ $enfant->Prenom_Ar }}" required>
                            </div>
            
                            <div class="mb-3">
                                <label for="Date_De_Naissance" class="form-label">Date de Naissance</label>
                                <input type="date" class="form-control" id="Date_De_Naissance" name="Date_De_Naissance" value="{{ $enfant->Date_De_Naissance }}" required>
                            </div>
                                ...
                                <div class="form-group">
                                    <select class="form-control" id="auxiliaire_id" name="auxiliaire_id" required>
                                        ...
                                        @foreach($auxiliaires as $auxiliaire)
                                            <option value="{{ $auxiliaire->id }}" {{ $enfant->auxiliaire_id == $auxiliaire->id ? 'selected' : '' }}>
                            ...
                            <button type="submit" class="btn btn-primary">Mettre à Jour l'Enfant</button>
                        </form>
                    </div>
                </div>
            </div>
