                ...
                <div class="navbar-nav w-100">
                    ...
                    <div class="nav-item dropdown">
                        ...
                        <div class="dropdown-menu bg-transparent border-0">
                            ...
                            <a href="{{route('auxiliaires.exportCard')}}" class="dropdown-item">Exportation Des Cartes</a></div>
                    </div>
...
<div class="container-fluid pt-4 px-4">
    <div class="bg-secondary rounded-top p-4">
        <h4>Détails de l'Entité Territoriale</h4>
    ...
    <div class="card text-white bg-dark m-4">
        <div class="card-header">
            <h4 class="text-center">{{ $entiteterritorielle->Nom }}</h4> 
        </div>
        <div class="card-body">
            
            <p class="card-text"><strong>Nom (Français):</strong> <br> {{ $entiteterritorielle->Nom }}</p>
            <p class="card-text"><strong>Nom (Arabe):</strong> <br> {{ $entiteterritorielle->Nom_Ar }}</p>
            <p class="card-text"><strong>Type:</strong> <br> {{ ucfirst($entiteterritorielle->type) }}</p>
            <p class="card-text"><strong>Gestionnaire:</strong> <br> {{ $entiteterritorielle->manager->Nom_Fr }}  {{ $entiteterritorielle->manager->Prenom_Fr }} --------  {{ $entiteterritorielle->manager->Nom_Ar }}  {{ $entiteterritorielle->manager->Prenom_Ar }}</p> <!-- Assuming a relation named `manager` -->
            <p class="card-text"><strong>Créé le:</strong> <br> {{ $entiteterritorielle->created_at->format('d/m/Y') }}</p>
            <p class="card-text"><strong>Mis à jour le:</strong> <br> {{ $entiteterritorielle->updated_at->format('d/m/Y') }}</p>
        </div>
         <!-- Table for listing Auxiliaires of this Entité Territoriale -->
         <div class="card-body">
            <h5 class="card-title">Auxiliaires associés à cette entité territoriale</h5>
            <a href="{{ route('auxiliaires.createKnownEntite',$entiteterritorielle->id) }}" class="btn btn-primary m-4">Ajouter un Auxiliaire</a>
            @if (count($entiteterritorielle->auxiliaires))
            ...
            @else
            <p>aucun auxiliaire n'est associé à cette entité territorielle.</p>
            @endif
            
        </div>
        

        <div class=" bg-secondary rounded card-footer d-flex justify-content-between">
            <a href="{{ route('entiteterritorielles.edit', $entiteterritorielle->id) }}" class="btn btn-warning m-4">Modifier</a>
            <form action="{{ route('entiteterritorielles.destroy', $entiteterritorielle->id) }}" method="POST" style="display: inline;" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer cette entité territoriale ?');">
                @csrf
                @method('DELETE')
            ...
            <a href="{{ route('entiteterritorielles.index') }}" class="btn btn-primary m-4">Retour à la liste</a>
        </div>
    </div>
