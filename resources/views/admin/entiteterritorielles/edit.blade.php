    ...
    <nav class="navbar navbar-expand bg-secondary navbar-dark sticky-top px-4 py-0">
        ...
        <div class="container-fluid pt-4 px-4">
            <div class="bg-secondary rounded-top p-4 d-flex justify-content-between align-items-center">
                <h4>Modifier une Entité Territoriale</h4>
                <a href="{{ route('entiteterritorielles.index') }}" class="btn btn-primary m-2">Annuler</a>
            </div>
        ...
        {{-- Formulaire pour modifier une entité territoriale --}}
        <form action="{{ route('entiteterritorielles.update', $entiteterritorielle->id) }}" method="POST">
            @csrf
            @method('PUT')
        
            <div class="form-group mb-3">
                <label for="Nom" class="col-sm-2 col-form-label">Nom (Français)</label>
                <div class="col-sm-10">
                    <input type="text" name="Nom" class="form-control" id="Nom" value="{{ old('Nom', $entiteterritorielle->Nom) }}" required>
                </div>
            </div>
            ...
            <div class="form-group mb-3">
                ...
                <div class="col-sm-10">
                    <input type="text" name="Nom_Ar" class="form-control" id="Nom_Ar" value="{{ old('Nom_Ar', $entiteterritorielle->Nom_Ar) }}" required>
                </div>
            </div>
            ...
            <div class="form-group mb-3">
                ...
                <div class="col-sm-10">
                    <select name="type" class="form-select" id="type" required>
                        <option value="Province" {{ old('type', $entiteterritorielle->type) == 'Province' ? 'selected' : '' }}>Province</option>
                        <option value="Pachalik" {{ old('type', $entiteterritorielle->type) == 'Pachalik' ? 'selected' : '' }}>Pachalik</option>
                        <option value="Caidat" {{ old('type', $entiteterritorielle->type) == 'Caidat' ? 'selected' : '' }}>Caidat</option>
                        <option value="Cercle" {{ old('type', $entiteterritorielle->type) == 'Cercle' ? 'selected' : '' }}>Cercle</option>
                        <option value="Annexe" {{ old('type', $entiteterritorielle->type) == 'Annexe' ? 'selected' : '' }}>Annexe</option>
                    </select>
                </div>
            </div>
        
            <div class="form-group mb-3">
                <label for="managed_by" class="col-sm-2 col-form-label">Gestionnaire</label>
                <div class="col-sm-10">
                    <select name="managed_by" class="form-select" id="managed_by" required >
                        @if(count($users))
                        <option value="">-- Sélectionnez un utilisateur --</option>
                        @foreach($users as $user)
                        <option value="{{ $user->id }}" {{ old('managed_by', $entiteterritorielle->managed_by) == $user->id ? 'selected' : '' }}>
                            {{ $user->Nom_Fr }} {{ $user->Prenom_Fr }} ----   {{ $user->Nom_Ar }} {{ $user->Prenom_Ar }}
                        </option>
                        @endforeach
                        @else 
                        <option value="">aucun utilisateur à selectionner</option>
                        @endif
                    </select>
                </div>
            </div>
