    ...
    <div class="container-fluid position-relative d-flex p-0">
        ...
        <div class="container-fluid pt-4 px-4">
            <div class="bg-secondary rounded-top p-4 d-flex justify-content-between align-items-center">
                <h4>Modifier Un Conjoint</h4>
                <a href="{{ route('conjoints.index') }}" class="btn btn-primary m-2">Annuler</a>
            </div>
        </div>
        ...
        {{-- Form to Edit an Existing Conjoint --}}
        <form action="{{ route('conjoints.update', $conjoint->id) }}" method="POST">
            @csrf
            @method('PUT')
        
            <div class="form-group mb-3">
                <label for="Nom_Fr">Nom (Français)</label>
                <input type="text" name="Nom_Fr" class="form-control" id="Nom_Fr" value="{{ old('Nom_Fr', $conjoint->Nom_Fr) }}" required>
            </div>
        
            <div class="form-group mb-3">
                <label for="Prenom_Fr">Prénom (Français)</label>
                <input type="text" name="Prenom_Fr" class="form-control" id="Prenom_Fr" value="{{ old('Prenom_Fr', $conjoint->Prenom_Fr) }}" required>
            </div>
        
            <div class="form-group mb-3">
                <label for="Nom_Ar">Nom (Arabe)</label>
                <input type="text" name="Nom_Ar" class="form-control" id="Nom_Ar" value="{{ old('Nom_Ar', $conjoint->Nom_Ar) }}" required>
            </div>
        
            <div class="form-group mb-3">
                <label for="Prenom_Ar">Prénom (Arabe)</label>
                <input type="text" name="Prenom_Ar" class="form-control" id="Prenom_Ar" value="{{ old('Prenom_Ar', $conjoint->Prenom_Ar) }}" required>
            </div>
        
            <div class="form-group mb-3">
                <label for="CNIE">CNIE</label>
                <input type="text" name="CNIE" class="form-control" id="CNIE" value="{{ old('CNIE', $conjoint->CNIE) }}" required>
            </div>
            ...
            <div class="form-group mb-3">
                ...
                <div class="col-sm-10">
                    <select name="auxiliaire_id" id="auxiliaire_id" class="form-select" required>
                        ...
                        <option value="{{ $auxiliaire->id }}" {{ (old('auxiliaire_id', $conjoint->auxiliaire_id) == $auxiliaire->id) ? 'selected' : '' }}>
