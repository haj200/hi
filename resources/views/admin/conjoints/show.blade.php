...
<body>
    <div class="container-fluid position-relative d-flex p-0">
            ...
            <nav class="navbar bg-secondary navbar-dark">
                ...
                <div class="navbar-nav w-100">
                    ...
                    <div class="nav-item dropdown">
                        <a href="" class="nav-link dropdown-toggle active" data-bs-toggle="dropdown">familles des auxiliaires</a>
        ...
        <div class="content">
            ...
            <div class="container-fluid pt-4 px-4 m-4">
                <div class="bg-secondary rounded-top p-4 d-flex justify-content-between align-items-center">
                    <h4>Détails du Conjoint</h4>
                    <a href="{{ route('conjoints.index') }}" class="btn btn-primary m-2">Retour</a>
                </div>
            </div>
                ...
                <div class="card m-4">
                    <div class="card-header bg-primary text-white">
                        <h5 class="mb-0">Informations du Conjoint</h5>
                    </div>
                    <div class="card-body">
                        <h5 class="card-title">{{ $conjoint->Nom_Fr }} {{ $conjoint->Prenom_Fr }}</h5>
                        <p class="card-text"><strong>Nom (Arabe):</strong> {{ $conjoint->Nom_Ar }}</p>
                        <p class="card-text"><strong>Prénom (Arabe):</strong> {{ $conjoint->Prenom_Ar }}</p>
                        <p class="card-text"><strong>CNIE:</strong> {{ $conjoint->CNIE }}</p>
                        <p class="card-text"><strong>Auxiliaire:</strong> {{ $conjoint->auxiliaire->Nom_Fr }} {{ $conjoint->auxiliaire->Prenom_Fr }}</p>
                    </div>
                    
                    <div class="bg-secondary  m-4 p-4 flex justify-content-between card-footer">
                        <a href="{{ route('conjoints.index') }}" class="btn btn-info m-4">retour à la liste</a>
                        <a href="{{ route('conjoints.edit', $conjoint->id) }}" class="btn btn-warning m-4">Modifier</a>
                        <form action="{{ route('conjoints.destroy', $conjoint->id) }}" method="POST" style="display: inline-block;">
                            @csrf
                            @method('DELETE')
