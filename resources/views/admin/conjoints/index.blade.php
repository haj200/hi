...
<body>
    <div class="container-fluid position-relative d-flex p-0">
        ...
        <div class="container-fluid m-4 pt-4 px-4">
            <div class="bg-secondary rounded-top p-4 d-flex justify-content-between align-items-center">
                <h4>Conjoints</h4>
                <a href="{{ route('conjoints.create') }}" class="btn btn-primary">Ajouter un Conjoint</a>
            </div>
        </div>
            ...
            <table class="table table-striped-columns">
                <thead>
                    <tr>
                        ...
                        <th scope="col">Prénom (Français)</th>
                        <th scope="col">Nom (Arabe)</th>
                        <th scope="col">Prénom (Arabe)</th>
                ...
                <tbody>
                    ...
                    <tr>
                        <th scope="row">{{ $conjoint->Nom_Fr }}</th>
                        <td>{{ $conjoint->Prenom_Fr }}</td>
                        <td>{{ $conjoint->Nom_Ar }}</td>
                        <td>{{ $conjoint->Prenom_Ar }}</td>
                        <td>{{ $conjoint->CNIE }}</td>
                        <td>
                            <a href="{{ route('conjoints.show', $conjoint->id) }}" class="btn btn-info ">Voir</a>
                            <a href="{{ route('conjoints.edit', $conjoint->id) }}" class="btn btn-light">Modifier</a>
                            <form action="{{ route('conjoints.destroy', $conjoint->id) }}" method="POST" style="display: inline-block;">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn btn-outline-danger" onclick="return confirm('Êtes-vous sûr de vouloir supprimer cet élement ?')">Supprimer</button>
                            </form>
                        </td>
                    </tr>
                    @endforeach
